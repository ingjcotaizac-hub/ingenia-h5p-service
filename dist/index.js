...
const H5P_ROOT = path_1.default.resolve(process.env.H5P_ROOT || path_1.default.join(process.cwd(), 'h5p-storage'));
...
app.use('/scorm/content/:scormId', async (req, res, next) => {
    ...
    const safeRelativePath = path_1.default.normalize(rawPath).replace(/^(\.\.[\/\\])+/, '');
    const localFilePath = path_1.default.join(H5P_ROOT, 'scorm', 'content', scormId, safeRelativePath);
    // 1. Si existe en el disco local del contenedor, servir directamente
    if (fs_1.default.existsSync(localFilePath) && fs_1.default.statSync(localFilePath).isFile()) {
        return res.sendFile(path_1.default.resolve(localFilePath));
    }
    // 2. Descargar bajo demanda desde Cloudflare R2
    const primaryBaseUrl = (process.env.R2_PUBLIC_URL || '').replace(/\/+$/, '');
    const fallbackBaseUrl = 'https://pub-f43f56500f3e4071b0bae7d0edfa84fc.r2.dev';
    const cleanRelative = safeRelativePath.replace(/\\/g, '/');
    const candidateUrls = [
        primaryBaseUrl ? `${primaryBaseUrl}/h5p-scorm/${scormId}/${cleanRelative}` : null,
        `${fallbackBaseUrl}/h5p-scorm/${scormId}/${cleanRelative}`
    ].filter(Boolean);
    let fileBuffer = null;
    ...
    for (const remoteUrl of candidateUrls) {
        try {
            ...
            if (r2Res.ok) {
                ...
                fileContentType = r2Res.headers.get('content-type') || fileContentType;
                break;
            }
        }
        catch {
    ...
    if (!fileBuffer) {
        console.warn(`[SCORM Content] Archivo no encontrado en R2: ${scormId}/${cleanRelative}`);
        return res.status(404).send('Not Found: File does not exist in SCORM package');
    }
    // Cachear en disco para servirlo instantáneamente en subsiguientes lecturas
    try {
        fs_1.default.mkdirSync(path_1.default.dirname(localFilePath), { recursive: true });
...
app.get('/scorm/play/:id', auth_1.requireAuth, async (req, res) => {
    ...
    const authCheck = await (0, authorizeContentAccess_1.authorizeContentAccess)(req.user, scormId, 'scorm', undefined, activityIdHint);
    if (!authCheck.ok) {
 ...
 */
async function syncContentToSupabase(contentId) {
    ...
    if (r2Config) {
        try {
            const result = await (0, r2Helper_1.uploadDirectoryToR2)(contentPath, storagePrefix, r2Config);
 ...
 */
async function restoreContentFromSupabase(contentId) {
    const contentPath = path_1.default.join(H5P_ROOT, 'content', String(contentId));
    // Si ya existen h5p.json y content.json en disco, no hace falta descargar
    if (fs_1.default.existsSync(path_1.default.join(contentPath, 'h5p.json')) && fs_1.default.existsSync(path_1.default.join(contentPath, 'content.json'))) {
        return true;
    }
    const storagePrefix = `h5p-drafts/${contentId}`;
    ...
    if (supabase) {
        try {
            ...
            if (remoteFiles && remoteFiles.length > 0) {
                fs_1.default.mkdirSync(contentPath, { recursive: true });
                for (const remoteFile of remoteFiles) {
                    const relativePath = remoteFile.substring(storagePrefix.length + 1);
                    const localFilePath = path_1.default.join(contentPath, relativePath);
                    fs_1.default.mkdirSync(path_1.default.dirname(localFilePath), { recursive: true });
    ...
    if (r2Config) {
        try {
            const r2Success = await (0, r2Helper_1.restoreDirectoryFromR2)(storagePrefix, contentPath, r2Config);
            if (r2Success) {
  ...
  ${POST_MESSAGE_SCRIPT}
    ...
    app.post('/h5p/editor/:contentId?', auth_1.requireAuth, (0, auth_1.requireRole)('teacher', 'admin'), upload.any(), async (req, res) => {
        try {
            ...
            syncContentToSupabase(String(newContentId)).catch(e => console.warn('[Sync] Error sincronizando contenido a Supabase:', e.message));
    ...
    app.use('/h5p/play/:contentId', auth_1.requireAuth, async (req, res, next) => {
        ...
        const activityIdHint = req.query.activityId ? String(req.query.activityId) : undefined;
        const authCheck = await (0, authorizeContentAccess_1.authorizeContentAccess)(req.user, contentId, 'h5p', undefined, activityIdHint);
        if (!authCheck.ok) {
            if (authCheck.status === 404) {
    ...
    app.get('/h5p/play/:contentId', async (req, res) => {
        try {
            ...
            await restoreContentFromSupabase(contentId);
            const model = await h5pPlayer.render(contentId, user);
    ...
    app.post('/api/upload-h5p', auth_1.requireAuth, (0, auth_1.requireRole)('teacher', 'admin'), upload.single('file'), async (req, res) => {
        try {
            ...
            syncContentToSupabase(String(result.id)).catch(e => console.warn('[Sync] Error sincronizando H5P subido a Supabase:', e.message));
